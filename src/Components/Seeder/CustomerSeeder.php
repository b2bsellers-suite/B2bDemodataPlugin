...
class CustomerSeeder
{
    ...
    public function __construct(ContainerInterface $container, AbstractSalesChannelContextFactory $contextFactory, Context $context)
    {
        ...
        /** @var RegisterRoute $salesChannelContextFactory */
    ...
    private function createCustomerDataBag($customerJson): RequestDataBag
    {
        ...
        $data->set('salutationId', $this->getSalutation($customerJson['salutation'])->getId());
        $data->set('firstName', $customerJson['firstName']);
        $data->set('lastName', $customerJson['lastName']);
    ...
    protected function getAddress($addressJson): RequestDataBag
    {
        return new RequestDataBag([
            ...
            'salutationId' => $this->getSalutation($addressJson['salutation'])->getId()
        ]);
    }
    ...
    private function createSalesRepRelations(CustomerEntity $customer, $customerJson)
    {
        if (isset($customerJson['isSalesRepOf']) && is_array($customerJson['isSalesRepOf'])) {
            foreach ($customerJson['isSalesRepOf'] as $salesRepRelation) {
                $relatedCustomer = $this->getCustomerByEmail($salesRepRelation['email']);
                if ($relatedCustomer) {
